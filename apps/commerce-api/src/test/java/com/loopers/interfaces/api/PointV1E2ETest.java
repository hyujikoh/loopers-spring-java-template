...
public class PointV1E2ETest {
    ...
    @Test
    @DisplayName("존재하는 유저가 여러 번 충전할 경우, 누적된 총 금액을 응답으로 반환한다.")
    void charge_multiple_times_returns_accumulated_amount() {
        ...
        assertAll(
                ...
                () -> assertThat(Objects.requireNonNull(firstResponse.getBody()).data().totalAmount()).isEqualByComparingTo(new BigDecimal("1000.00"))
        );
        ...
        assertAll(
                ...
                () -> assertThat(Objects.requireNonNull(secondResponse.getBody()).data().username()).isEqualTo(userCommand.username()),
                () -> assertThat(Objects.requireNonNull(secondResponse.getBody()).data().totalAmount()).isEqualByComparingTo(new BigDecimal("3500.00"))
        );
        ...
        assertAll(
                ...
                () -> assertThat(Objects.requireNonNull(thirdResponse.getBody()).data().username()).isEqualTo(userCommand.username()),
                () -> assertThat(Objects.requireNonNull(thirdResponse.getBody()).data().totalAmount()).isEqualByComparingTo(new BigDecimal("4000.00"))
        );

        // 최종 포인트 조회로 검증
        ParameterizedTypeReference<ApiResponse<PointV1Dtos.PointInfoResponse>> pointInfoResponseType = new ParameterizedTypeReference<>() {
        };
        ...
        assertAll(
                ...
                () -> assertThat(Objects.requireNonNull(pointInfoResponse.getBody()).data().username()).isEqualTo(userCommand.username()),
                () -> assertThat(Objects.requireNonNull(pointInfoResponse.getBody()).data().currentPointAmount()).isEqualByComparingTo(new BigDecimal("4000.00"))
        );
    }
