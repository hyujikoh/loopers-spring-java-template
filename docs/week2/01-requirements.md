...
### 사용자 식별
- 모든 API는 `X-USER-ID` 헤더를 통해 사용자를 식별
- 별도 인증/인가 로직 없음
...
- 존재하지 않는 사용자의 좋아요 목록 조회 시 404 Not Found 응답
...
- 일부 상품만 주문되는 상황 방지 (전체 성공 또는 전체 실패)
