...
## 시나리오 5 : 쿠폰 관리 및 적용
### 유저 스토리
"할인 쿠폰을 사용해서 주문 가격을 절감하고 싶다"

상황: 쿠폰을 보유 중이며 주문 시 할인을 받고 싶은 경우 니즈:
- 쿠폰으로 얼마나 할인받을 수 있는지 미리 확인하고 싶음
- 주문 시 쿠폰을 적용해서 결제하고 싶음
- 쿠폰 사용 후 다시 사용하려고 할 때 실수로 중복 사용되지 않았으면 함

### 기능 요구사항
1. 쿠폰 타입
   정액 쿠폰: 고정 금액 할인 (예: 5,000원 할인)
   배율 쿠폰: 주문 금액의 일정 비율 할인 (예: 10% 할인)
2. 쿠폰 1회성 및 사용 제한
   쿠폰은 1회성으로 한 번 사용하면 재사용 불가
   이미 사용한 쿠폰은 다시 사용할 수 없음
   사용 중인 쿠폰을 재사용 시도 시 명확한 에러 메시지 반환
3. 주문 시 쿠폰 적용 (설계 검토 대상)
   현재 검토 중인 적용 방식:
   방식 A: 상품별 쿠폰 적용 - 각 상품마다 개별적으로 쿠폰 적용
   방식 B: 주문 전체에 쿠폰 적용 - 1개 주문에 1개 쿠폰 적용
   임시 결정: 주문 단위로 1건의 쿠폰만 적용 가능
   1개 주문에 최대 1개의 쿠폰 적용
   여러 쿠폰을 조합해서 사용하는 기능은 추후 검토 예정
4. 동시성 제어
   동시에 같은 쿠폰으로 여러 주문을 시도해도 1건만 사용 성공
   첫 번째 사용자의 주문만 쿠폰 적용되고, 나머지는 쿠폰 사용 불가 에러 반환
   쿠폰의 일회성을 보장하기 위한 원자적 처리
5. 쿠폰 조회
   사용자의 보유 쿠폰 목록 조회 가능
   각 쿠폰의 정보 (타입, 할인율/금액, 사용 여부) 확인 가능
   이미 사용한 쿠폰은 구분 가능해야 함
6. 오류 처리
   존재하지 않는 쿠폰 사용 시도 시 404 Not Found 응답
   이미 사용된 쿠폰 재사용 시도 시 400 Bad Request 응답
   사용 권한이 없는 쿠폰 사용 시도 시 403 Forbidden 응답
   기술 구현 요구사항
   쿠폰 타입: FIXED_AMOUNT (정액), PERCENTAGE (배율)
   쿠폰 상태: UNUSED (미사용), USED (사용됨)
   트랜잭션: 쿠폰 사용 상태 변경과 주문 생성의 원자성
   동시성 제어: 쿠폰의 일회성 보장을 위한 낙관적/비관적 잠금 처리
   검증: 주문 생성 시 쿠폰 유효성 검증 (존재 여부, 사용 가능 여부, 보유자 확인)